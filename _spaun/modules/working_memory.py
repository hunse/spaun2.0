...
class WorkingMemory(Module):
    ...
    def init_module(self):
        # Memory input node
        self.mem_in = nengo.Node(size_in=self.cfg.sp_dim, label='WM Module In Node')

        sp_add_matrix = (vocab['ADD'].get_convolution_matrix() *
                         (0.25 + 0.25 / self.cfg.mb_decaybuf_input_scale))
        # sp_add_matrix = (vocab['ADD'].get_convolution_matrix() * 0.5)

        self.num0_bias_node = nengo.Node(vocab.parse('POS1*ZER').v,
                                         label="POS1*ZER")

        self.gate_sig_bias = self.cfg.make_thresh_ens_net(label='Gate Sig Bias')
        # Bias the -1.5 neg_atn

        self.cnt_gate_sig = self.cfg.make_thresh_ens_net(0.5, label='Cnt Gate Sig')
    ...
    def setup_connections(self, parent_net):
        ...
        if hasattr(p_net, 'ps'):
            ...
            nengo.Connection(ps_state_mb_utils[mb2_no_gate_inds],
                             self.mb2_net.gate,
                             transform=[[-self.cfg.mb_gate_scale * 2] *
...
class WorkingMemoryDummy(WorkingMemory):
    ...
    def init_module(self):
        ...
        self.sel_mb1_in = self.cfg.make_selector(3, default_sel=0, n_ensembles=1,
        ...
        self.sel_mb2_in = self.cfg.make_selector(3, default_sel=0, n_ensembles=1,
                                            ens_dimensions=self.cfg.sp_dim,
                                            n_neurons=self.cfg.sp_dim)
        ...
        self.sel_mb3_in = self.cfg.make_selector(3, default_sel=0, n_ensembles=1,
                                            ens_dimensions=self.cfg.sp_dim,
                                            n_neurons=self.cfg.sp_dim)
        nengo.Connection(self.mem_in, self.sel_mb3_in.input0, synapse=None)

        # Memory block Ave (MBAve)
        self.mbave_in = nengo.Node(size_in=self.cfg.sp_dim)
