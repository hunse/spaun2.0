...
class VisionSystem(Module):
    def __init__(self, cfg, label="Vision Sys", seed=None, add_to_container=None,
    ...
    def init_module(self, vis_net, detect_net, vis_sps, vis_sps_scale,
        ...
        if detect_net is None:
            detect_net = DetectChange(dimensions=images_data_dimensions,
                                      n_neurons=self.cfg.n_neurons_ens)
        ...
        self.am = self.cfg.make_assoc_mem(vis_sps, vis_vocab.vectors,
        ...
        #                      vis_net)
        self.cfg.vis_dim = vis_sps.shape[1]
        vis_sp_vocab = Vocabulary(self.cfg.vis_dim)  # TODO: FIX THIS?

        self.vis_mb = MB(self.cfg.n_neurons_mb * 2, self.cfg.vis_dim, gate_mode=2,
                         vocab=vis_sp_vocab, radius=vis_sps_scale,
                         **self.cfg.mb_config)
        nengo.Connection(
            self.am.cleaned_output_utilities[item_mb_gate_sp_inds],
            self.vis_mb.gate, transform=[[self.cfg.mb_gate_scale] *
...
class VisionSystemDummy(VisionSystem):
    def __init__(self, cfg, label="Dummy Vision Sys", seed=None,
    ...
    def dummy_detect_net(self):
        return DetectChange(None, dimensions=self.cfg.sp_dim,
                            item_magnitude=(self.cfg.get_optimal_sp_radius() / 2.0))
